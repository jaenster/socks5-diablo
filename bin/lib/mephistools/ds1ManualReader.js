...
class Ds1 {
	static loadFile(basePath, filename, objectsTypes) {
		const fpath = basePath + '\\' + filename.replace(/\//g, '\\');
		const bytes = fs.readFileSync(fpath);
		const ds1 = Ds1.loadBytes(basePath, bytes, objectsTypes);
		ds1.filename = fpath;
		return ds1
	}

	static loadBytes(basePath, bytes, objectsTypes) {
		let offset = 0;
		const ds1 = new Ds1();
		...
		offset = Ds1.readObjects(basePath, ds1, bytes, offset, act, objectsTypes);
	...
	static readObjects(basePath, ds1, bytes, offset, act, objectsTypes) {
		if (ds1.version < 2) {
			return offset
		}
		...
		*/
		...
		for (let i = 0; i < objectCount; i++) {
			const info = {};
			/* const type = */
			bytes.readInt32LE(offset); // type
			offset += 4;
			const id = bytes.readInt32LE(offset); // id
			offset += 4;
			...
			// console.log(type, id, info.x, info.y)
			// info.preset = SpawnPreset.Find(act, type, id)
			// ds1.objects[i] = info
			// console.log(d)

			info.preset = {};
			info.preset = objectsTypes.instances.find(j => j.Act === act && j.Id === id); /* && j['SizeX'].includes(info.x) && j['SizeY'].includes(info.y) */
			// console.log(info.preset)
			ds1.objects[i] = info
		}
		// console.log(ds1.objects)
		return offset
	}
